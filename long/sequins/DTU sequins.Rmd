...
res.all <- data.frame(
  ...
  test = c(rep("DRIMSeq_gene", nrow(res.gene)), rep("DRIMSeq_feature", nrow(res.tx)), rep("stageR_gene", length(unique(drim.padj$geneID))), rep("stageR_transcript", nrow(drim.padj))),
  level = c(rep("Gene level", nrow(res.gene)), rep("Feature level", nrow(res.tx)), rep("Gene level", length(unique(drim.padj$geneID))), rep("Feature level", nrow(drim.padj)))
)
...
ds.all <- data.frame(
  ...
  DTU = c(proportion$isChanges[match(ts$GeneID, proportion$geneID)], proportion$isChanges[match(ts.f$GeneID, proportion$geneID)],
  ...
  level = c(rep("Gene level", nrow(ts)), rep("Gene level", nrow(ts.f)), rep("Feature level", nrow(ts.tx)))
)
...
pdf("DTU_FDR.pdf", height = 5, width = 8)
...
```{r}
pdf("DTU_TPR.pdf", width = 8, height = 5)
