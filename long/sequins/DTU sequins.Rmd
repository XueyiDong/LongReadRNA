...
colnames(annoData)[5:8] <- paste0("barcode0", c(3, 4, 1, 2))
annoData <- melt(annoData, id.vars = 1:4)
annoData$exp <-annoData$MIX_A
annoData$exp[annoData$variable %in% c("barcode03", "barcode04")] <- annoData$MIX_B[annoData$variable %in% c("barcode03", "barcode04")]
colnames(annoData)[5] <- "sample"


library(ggplot2)
pdf("countsVsAnno.pdf", height = 5, width = 8)
...
# set stager transcript level NA to 1
...
res.all <- data.frame(
  ...
  adj_pvalue = c(res.gene$adj_pvalue, res.tx$adj_pvalue, drim.padj$gene[a], drim.padj$transcript),
  isChanges = c(res.gene$isChanges, res.tx$isChanges, drim.padj$isChanges[a], drim.padj$isChanges),
  method = c(rep("DRIMSeq_gene", nrow(res.gene)), rep("DRIMSeq_feature", nrow(res.tx)), rep("stageR_gene", length(unique(drim.padj$geneID))), rep("stageR_transcript", nrow(drim.padj)))
)

ggplot(na.omit(res.all), aes(x=isChanges, y=adj_pvalue, fill=method)) +
  geom_boxplot() +
  theme_bw()
