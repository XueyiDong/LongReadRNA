...
summary(filter)
x <- x[filter, keep.lib.sizes=TRUE]
x <- calcNormFactors(x)
lcpm <- cpm(x, log=TRUE)
...
legend("bottomright", c("Smchd1-null", "wt"), text.col = col[c(1,2)], cex = 1.25)
dev.off()
```

```{r}
x$samples$groups <- x$samples$group
x$samples$groups <- gsub("smchd1", "Smchd1-null", x$samples$groups)
x$samples$groups <- gsub("wt", "WT", x$samples$groups)
pdf("plots/libsize.pdf", height = 4, width = 8)
library(ggplot2)
ggplot(x$samples, aes(x=sample, y=lib.size, fill=groups)) +
...
ggplot(tt, aes(x=length, y=AveExpr)) +
  ...
  annotate(geom="text", x=40000, y=14, label=expression(paste(rho, "=-0.10")), size=8) +
...
pdf("plots/md.pdf", height = 5, width = 8)
  plotMD(efit, status=dt, column=1, values=c(1,-1), hl.col=c("red", "blue"), legend=FALSE, main=NA,
         cex.lab = 1.5, cex.axis = 1.5)
  # highlight smchd1
  points(efit$Amean[sm], efit$coefficients[sm, 1], col = "green", pch = 16, cex = 1)
  points(efit$Amean[imprinted], efit$coefficients[imprinted, 1], col = "orange", pch = 16, cex = 1)
  legend("topright", legend = c("Imprinted", "Up", "NotSig", "Down", "Smchd1"),
         col = c("orange","red", "black", "blue", "green"),
         cex = 1.2, 
         pch = c(16, 16, 16, 16, 16),
         pt.cex = c(1, 1, 0.3, 1, 1))
dev.off()
```
