...
# set stager transcript level NA to 1？ no
suppressWarnings({
...
res.all <- data.frame(
  id = c(res.gene$gene_id, res.tx$feature_id, unique(as.character(drim.padj$geneID)), as.character(drim.padj$txID)),
...
ds.all <- data.frame(
  id = c(ts$GeneID, ts.f$GeneID, ts.tx$ExonID, 
         unique(as.character(ds.padj$geneID)), as.character(ds.padj$txID)
         # , 
...
topN.ds <- ggplot(top47, aes(x=number, y=intersect)) + geom_line() +
...
calculate the overlap of top N genes of DRIMSeq gene results
...
# pdf("plots/drimTopN.pdf", height = 5, width = 8)
topN.drim <- ggplot(top47.drim, aes(x=number, y=intersect)) + geom_line() +
  geom_abline(colour="grey") +
  theme_bw() +
  theme(text = element_text(size=20), legend.text = element_text(size=14)) +
  labs(x = "Top n genes", y = "Number of intersect") +
...
```{r}
proportionA <- data.frame(
  transcript = rep(proportion$txID, 2),
  proportion_A = rep(proportion$MIX_A, 2),
  proportion_estimated = c(proportions(d)$OXBCHWNYNTPSSTGN[match(proportion$txID, proportions(d)$feature_id)], proportions.short$X1.H1975.1_S1[match(proportion$txID, proportions.short$feature_id)]),
  dataset = rep(c("long read", "short read"), c(nrow(proportion), nrow(proportion))),
  abundance = rep(anno$MIX_A, 2)
)

pdf("plots/proportionA.pdf", width = 8, height = 5)
ggplot(proportionA, aes(x=proportion_A, y=proportion_estimated, colour=dataset)) +
  geom_point() +
  geom_smooth(method="lm", formula=y~x)+
  labs(x = "Proportion expected", y = "proportion estimated") +
  theme_bw() +
  theme(text = element_text(size=20), legend.text = element_text(size=14))
dev.off()
```
...
annoDb <- makeTxDbFromGFF(gff)
tr <- GeneRegionTrack(annoDb, chromosome = "chrIS", start = 8945052, end = 9015138)
gtrack <- GenomeAxisTrack()
plotTracks(list(gtrack, tr), transcriptAnnotation="transcript", lwd = 1, lwd.grid = 1)
```

```{r}
library(data.table)
anno63 <- anno[anno$NAME == "R1_63_1" | anno$NAME == "R1_63_2", ]
anno63 <- melt(anno63, id = c("NAME", "LENGTH"))
colnames(anno63)<- c("Transcript", "Length", "Group", "Abundance") 
anno63$Transcript <- factor(anno63$Transcript, levels = c("R1_63_2", "R1_63_1"))

ggplot(anno63, aes(x=Transcript, y=Abundance, fill=Group)) +
  geom_bar(stat="identity", position="dodge") +
  theme_bw() +
  theme(text = element_text(size=20), legend.text = element_text(size=14)) 

# library(cowplot)
# plot_grid(tracks, prop63, ncol=2)
```
